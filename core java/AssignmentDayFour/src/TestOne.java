import com.zensar.Employee;
import com.zensar.Manager;
import com.zensar.MarketingExecutive;
;
//main class of que 7
public class TestOne {
	public static void printObjects(Employee e)
	{
		e.display();
	}
	public static void main(String args[])
	{
		Employee employee=new Employee(113,"aashi",325);
		printObjects(employee);
		Manager manager=new Manager(112,"nishi",12000);
		printObjects(manager);
		MarketingExecutive marketingExecutive=new MarketingExecutive(111,"Akanksha",10000,20);
		printObjects(marketingExecutive);
		
		
	/*	MarketingExecutive marketingExecutive=new MarketingExecutive(111,"Akshay",10000,20);
		Manager manager=new Manager(112,"Jack",12000);
		marketingExecutive.GrossSalaryME();
		marketingExecutive.NetSalaryMS();
		marketingExecutive.display();

		manager.GrossSalaryM();
		manager.NetSalaryM();
		manager.display();
	*/
	}

}
